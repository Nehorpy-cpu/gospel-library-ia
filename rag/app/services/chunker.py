...
class SmartChunker:
    ...
    def chunk(self, text: str) -> list[ChunkCandidate]:
        ...
        for block, start, end, section in blocks:
            ...
            if block_tokens > self.settings.chunk_max_tokens:
                if current:
            ...
            if current and current_tokens + block_tokens > self.settings.chunk_target_tokens:
                chunks.append(self._build_chunk(len(chunks), current))
    ...
    def _split_large_block(self, block: str, absolute_start: int) -> list[tuple[str, int, int]]:
        ...
        for sentence in sentences:
            ...
            if current and current_tokens + tokens > self.settings.chunk_target_tokens:
    ...
    def _overlap_tail(self, current: list[tuple[str, int, int, str | None]]):
        ...
        for item in reversed(current):
            ...
            if tokens + item_tokens > self.settings.chunk_overlap_tokens:
                break
            tail.insert(0, item)
            tokens += item_tokens
        return tail
    ...
    def _single_chunk(self, index: int, text: str, start: int, end: int, section: str | None):
        text = trim_to_tokens(text, self.settings.chunk_max_tokens)
